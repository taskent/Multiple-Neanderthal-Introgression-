...
outf = open(outputfile1, 'w')
outf2 = open(outputfile2, 'w')

	
	
	
#Get individuals, prop of derived alleles for Sstar significant SNPs per phased haplotype, and number of introgressed haplotypes 	
def get_inds_noHaplos(line): #line from the phased haplotypes input file (inf1)
	N = line[8]
	inds = line[6].split(';')
	return inds, N
...
def get_ind_col_id_list(line): #line from the phased haplotypes input file (inf1) 
	...
	ind_column_id_l = []
	inds_haplos = line[6].split(';') 
	ind_l = []
	for ind_haplo in inds_haplos:
		ind = ind_haplo.split('_')[0]
		ind_l.append(ind)
	for k in ind_l:
...
def get_ind_hapl_list(line): #line from the phased haplotypes input file (inf1) 
	...
	inds_haplos = line[6].split(';') 
	for ind_haplo in inds_haplos:
		haplo_no = ind_haplo.split('_')[1][-1]
...
def get_eurasian_der_al_freq(population, line): #line from the genotypes input file (inf2)
	...
	if population == 'easn':
		...
		for i in index_list:
			...
			if geno2 == '1': 
				count += 1
		freq = float(count)/ (len(index_list) * 2.0)
		...
		index_list = [496, 497, 498, 499, 500, 501, 502, 503, 504, 505, 506, 507, 508, 509, 510, 511, 512, 513, 514, 612, 613, 614, 615, 616, 617, 618, 619, 620, 621, 622, 623, 624, 625, 626, 627, 628, 629, 630, 631, 632, 730, 731, 732, 733, 734, 735, 736, 737, 738, 739, 740, 741, 742, 743, 744, 745, 746, 747, 748, 749, 750, 751, 752, 753, 754, 755, 756, 757, 758, 759, 760, 761, 762, 763, 764, 765, 766, 767, 768, 769, 770, 771, 772, 773, 774, 775, 776, 777]
		count = 0.0
		for i in index_list:
			genos = line[i+5].split('|')
			geno1 = genos[0]
...
def get_hominin_der_al_count_freq(hominin, line): #line from the genotypes input file (inf2)
	...
	genos = index.split('/')
	#print ('genos: ', genos)
	#print ('genos.count(1): ', genos.count('1'))
...
def get_pi_hominin_hominin(hominin1, hominin2, line): #line from the genotypes input file (inf2)
	...
	if hominin1_der_allele_freq == 1: 
		pi = 1.0 - float(hominin2_der_allele_freq)
	elif hominin2_der_allele_freq == 1:
...
def Sstar_snp_sharing(population, line, line2, snp_pos_shared_dict, snp_pos_population_freq_dict, notSstar_snp_pos_shared_dict, notSstar_snp_pos_population_freq_dict, notSstar_snp_pos_noYri_shared_dict, notSstar_snp_pos_noYri_population_freq_dict, notSstar_snp_pos_noYri_count_no_der_in_intr_hapl_dict, per_haplo_noSNPs_nonSstar_noYri_total_haplo_no_corrected, per_haplo_noSNPs_nonSstar_noYri_count): #line: from the phased haplotypes input file (inf1), line2: from the genotypes input file (inf2)
	...
	ea_freq = [ str(get_eurasian_der_al_freq('easn', line2) )]
	we_freq = [ str(get_eurasian_der_al_freq('nweur', line2) )]
	Yri_freq = [ str( get_eurasian_der_al_freq('yri', line2))]
	freqs = ';'.join(ea_freq + we_freq + Yri_freq)
	sharing = ''
	if altai_freq != 0:
		sharing += 'Altai' + '_'
	if vindija_freq != 0:
	...
	if snp in snps:	
		...
		snp_pos_population_freq_dict[pos] = freqs		
	elif snp not in snps:
		notSstar_snp_pos_shared_dict[pos] = sharing
		notSstar_snp_pos_population_freq_dict[pos] = freqs	
		if yri_freq == 0.0:
...
def main():
	...
	count_value_error = 0 
	count_name_error = 0 
	count_easn_sp_Sstar_snps = 0
	count_eur_sp_Sstar_snps = 0
	for Line in inf:
		l = Line.split()
		try:
			chr = l[0]
			...
			individuals = get_inds_noHaplos(l)[0]
			no_haplos = get_inds_noHaplos(l)[1]
		except IndexError:
			count_index_error += 1
			continue
		except ValueError:
		...
		total_no_snps_per_introg_haplo = 0.0
		total_no_snps_for_Wattersons_theta = 0.0
		
		no_of_altai_sp_snps = 0.0
		no_of_vindija_sp_snps = 0.0
		no_of_denis_sp_snps = 0.0
		no_of_altai_vind_sh_snps = 0.0
		no_of_altai_denis_sh_snps = 0.0
		no_of_vind_denis_sh_snps = 0.0
		no_of_altai_vind_denis_sh_snps = 0.0
		no_of_not_sh_snps = 0.0
		...
		for line2 in inf2:
			...
			if  int(end2) > int(newSNPs[-1]):
				...
				altai_dr_al_freq = get_hominin_der_al_count_freq('altai', l2)[1]				
				vindija_dr_al_freq = get_hominin_der_al_count_freq('vindija', l2)[1]				
				chagyrskaya_dr_al_freq = get_hominin_der_al_count_freq('chagyrskaya', l2)[1]				
				denisova_dr_al_freq = get_hominin_der_al_count_freq('denisova', l2)[1]	
				introg_der_al_freq = get_introg_der_al_count_freq(l, l2)[2]

				if introg_der_al_freq > 0: 
					if altai_dr_al_freq > 0:
						if vindija_dr_al_freq > 0:
							if denisova_dr_al_freq > 0:
								no_of_altai_vind_denis_sh_snps += 1
							else:
								no_of_altai_vind_sh_snps += 1								
						else:
							if denisova_dr_al_freq > 0:
								no_of_altai_denis_sh_snps += 1
							else:
								no_of_altai_sp_snps += 1								
					else:
						if vindija_dr_al_freq > 0:
							if denisova_dr_al_freq > 0:
								no_of_vind_denis_sh_snps += 1
							else:
								no_of_vindija_sp_snps += 1		
						else:
							if denisova_dr_al_freq > 0:
								no_of_denis_sp_snps += 1	
							else:
								no_of_not_sh_snps += 1	

				Sstar_snp_sharing('nweur', l, l2, snp_pos_shared_d, snp_pos_population_freq_d, notSstar_snp_pos_shared_d, notSstar_snp_pos_population_freq_d, notSstar_snp_pos_noYri_shared_d, notSstar_snp_pos_noYri_population_freq_d, notSstar_snp_pos_noYri_count_no_der_in_intr_hapl_d, per_haplo_noSNPs_nonSstar_noYri_total_haplo_no_corr, per_haplo_noSNPs_nonSstar_noYri)
				
				if end2 in newSNPs:
					easn_al_freq = float(get_eurasian_der_al_freq('easn', l2))
					eur_al_freq = float(get_eurasian_der_al_freq('nweur', l2))
					yri_dr_al_freq = float(get_eurasian_der_al_freq('yri', l2))	
					if eur_al_freq < 0.01 and easn_al_freq > 0.05 and yri_dr_al_freq < 0.01 and check != 0:
						count_easn_sp_Sstar_snps += 1
						check = 1
					if eur_al_freq > 0.05 and easn_al_freq < 0.01 and yri_dr_al_freq < 0.01 and check2 != 0:
						count_eur_sp_Sstar_snps += 1
						check2 = 1
				...
				pi_nonintrog_nonintrog = get_pi_nonintrog_nonintrog('nweur', l, l2)
				total_pi_nonintrog_nonintrog += pi_nonintrog_nonintrog 
				pi_altai_nonintrogressed = get_pi_hominin_nonintrog('altai', 'nweur', l, l2)
				total_pi_altai_nonintrogressed += pi_altai_nonintrogressed 
				pi_vindija_nonintrogressed = get_pi_hominin_nonintrog('vindija', 'nweur', l, l2)
				total_pi_vindija_nonintrogressed += pi_vindija_nonintrogressed
				pi_chagyrskaya_nonintrogressed = get_pi_hominin_nonintrog('chagyrskaya', 'nweur', l, l2)
				total_pi_chagyrskaya_nonintrogressed += pi_chagyrskaya_nonintrogressed			 	
				pi_denisova_nonintrogressed = get_pi_hominin_nonintrog('denisova', 'nweur', l, l2)
				total_pi_denisova_nonintrogressed += pi_denisova_nonintrogressed
				pi_introg_nonintrogressed = get_pi_introg_nonintrog('nweur', l, l2)
				total_pi_introg_nonintrog += pi_introg_nonintrogressed
				#print('len_l: ', len_l)
				total_no_snps_per_introg_haplo += get_introg_der_al_count_freq(l, l2)[0]
		
				if get_introg_der_al_count_freq(l, l2)[2] > 0 or altai_dr_al_freq > 0 or vindija_dr_al_freq > 0:
					total_no_snps_for_Wattersons_theta += 1
		...
		if count_w_snps > 0:
			...
			individuals = [ str(';'.join(individuals))]
			no_haplos = [str(no_haplos)]
			
			total_no_snps_per_introghaplo = [str(total_no_snps_per_introg_haplo)]
			total_no_snps_for_Wattersonstheta = [str(total_no_snps_for_Wattersons_theta)]

			no_of_altai_sp_Snps = [ str(no_of_altai_sp_snps) ] 
			no_of_vindija_sp_Snps = [ str(no_of_vindija_sp_snps) ]
			no_of_denis_sp_Snps = [ str(no_of_denis_sp_snps) ]
			no_of_altai_vind_sh_Snps = [ str(no_of_altai_vind_sh_snps) ]
			no_of_altai_denis_sh_Snps = [ str(no_of_altai_denis_sh_snps) ] 
			no_of_vind_denis_sh_Snps = [ str(no_of_vind_denis_sh_snps) ] 
			no_of_altai_vind_denis_sh_Snps = [ str(no_of_altai_vind_denis_sh_snps) ]
			no_of_not_sh_Snps = [ str(no_of_not_sh_snps) ] 
	
			#print ('\t'.join(l[:4] + individuals + percent_derived_avr + no_haplos + pi_introg_altai + pi_introg_vindija + pi_introg_nonintrog + pi_nonintrog_nonintrog + pi_altai_nonintrogressed + pi_vindija_nonintrogressed + pi_altai_vindija + pi_introg_introg + pi_altai_denisova + pi_vindija_denisova + pi_introg_denisova + pi_denisova_nonintrogressed + pi_altai_chagyrskaya + pi_vindija_chagyrskaya + pi_chagyrskaya_denisova + pi_introg_chagyrskaya + pi_chagyrskaya_nonintrogressed + total_no_snps_per_introghaplo), file = outf  )	
			#print ('\t'.join(l[:4] + individuals + percent_derived_avr + no_haplos + notSstar_snp_pos_shared_d + notSstar_snp_pos_population_freq_d + notSstar_snp_pos_noYri_shared_d + notSstar_snp_pos_noYri_population_freq_d + notSstar_snp_pos_noYri_count_no_der_in_intr_hapl_d + per_haplo_noSNPs_nonSstar + per_haplo_noSNPs_nonSstar_noYri + per_haplo_noSNPs_nonSstar_noYri_total_haplo_no_corr), file = outf2 )
					
			print >>outf, '\t'.join(l[:4] + individuals + no_haplos + pi_introg_altai + pi_introg_vindija + pi_introg_nonintrog + pi_nonintrog_nonintrog + pi_altai_nonintrogressed + pi_vindija_nonintrogressed + pi_altai_vindija + pi_introg_introg + pi_altai_denisova + pi_vindija_denisova + pi_introg_denisova + pi_denisova_nonintrogressed + pi_altai_chagyrskaya + pi_vindija_chagyrskaya + pi_chagyrskaya_denisova + pi_introg_chagyrskaya + pi_chagyrskaya_nonintrogressed + total_no_snps_per_introghaplo + total_no_snps_for_Wattersonstheta + no_of_altai_sp_Snps + no_of_vindija_sp_Snps + no_of_denis_sp_Snps  + no_of_altai_vind_sh_Snps + no_of_altai_denis_sh_Snps + no_of_vind_denis_sh_Snps + no_of_altai_vind_denis_sh_Snps + no_of_not_sh_Snps)
			print >>outf2, '\t'.join(l[:4] + individuals + no_haplos + snp_pos_shared_d + snp_pos_population_freq_d + notSstar_snp_pos_shared_d + notSstar_snp_pos_population_freq_d + per_haplo_noSNPs_nonSstar_noYri + per_haplo_noSNPs_nonSstar_noYri_total_haplo_no_corr)
	...
	outf.close()
	outf2.close()
	print 'Number of EASN specific Sstar SNPs: ', count_easn_sp_Sstar_snps
	print 'Number of EUR specific Sstar SNPs: ', count_eur_sp_Sstar_snps
