##Sstats -Sliding Windows of size 50 kb at each 20 kb- eur-YRI -second half - Reference Chimp
...
def get_eur_yri_col_nos(genotypes_start_col, eur_sample_size, yri_sample_size):
	eur_col_no_l = []
	for i in range(genotypes_start_col, genotypes_start_col + eur_sample_size):
		eur_col_no_l.append(i)
	yri_col_no_l = []
	for i in range(genotypes_start_col + eur_sample_size, genotypes_start_col + eur_sample_size + yri_sample_size):
		yri_col_no_l.append(i)
	return eur_col_no_l, yri_col_no_l
...
def get_window_start_end_line_no_dicts(infile):
	...
	#inf_wind = open("/projects/academic/omergokc/ozgur_Sstats/windows/winds_Sstats.chr." + CHRX + ".txt", 'r')
	wind_start_snp_dict = {}
	wind_end_snp_dict = {}
	count_w = 0
...
def main():
	...
	eur = get_eur_yri_col_nos(genotypes_start_col, eur_sample_size, yri_sample_size)[0]
	afr = get_eur_yri_col_nos(genotypes_start_col, eur_sample_size, yri_sample_size)[1]
	...
	for j in range(len(Wind_start_line_no_dict)):  ##for each window with the ascending order:
		...
		for m in eur: 	#for all 20 individuals from East Asia
			#print m
